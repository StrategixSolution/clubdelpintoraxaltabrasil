...
class Ventas_auditoria_segunda_controller extends Base_Controller {
    public function __construct(){ 
        ...
        $this->load->model('ventas/ventas_auditoria/ventas_auditoria_segunda/ventas_auditoria_segunda_model');
    ...
    public function index(){//Pagina de Inicio
        $this->base_controller_create_view_sistema('ventas/ventas_auditoria/ventas_auditoria_segunda/ventas_auditoria_segunda_form_view');
    }
    public function ventas_auditoria_segunda_controller_combo_anio() {
        $cmbAnio ="<option  value='0'>".$this->lang->line('ventas_auditoria_segunda_controller_lang_placeholder_anio')."</option>";
        $anios  = $this->ventas_auditoria_segunda_model->ventas_auditoria_segunda_model_combo_anio();
        foreach ($anios as $anio) {            
            $cmbAnio .="<option value=$anio->anio>$anio->anio</option>";
        }
        echo json_encode($cmbAnio);
    }
    public function ventas_auditoria_segunda_controller_combo_mes() {
        $cmbAnio = $this->input->post('anio',true);
        $cmbMes ="<option  value='0'>".$this->lang->line('ventas_auditoria_segunda_controller_lang_placeholder_mes')."</option>";
        $meses  = $this->ventas_auditoria_segunda_model->ventas_auditoria_segunda_model_combo_mes($cmbAnio);
        foreach ($meses as $mes) {            
            $cmbMes .="<option value=$mes->mes>".strtoupper(funciones_strategix_mes_numero_texto($mes->mes))."</option>";
        }
        echo json_encode($cmbMes);
    }
    public function ventas_auditoria_segunda_controller_tabla() {
        $cmbAnio     = $this->input->post('anio',true);
        $cmbMes      = $this->input->post('mes',true); $i=1;$lista ="";
        $ventas      = $this->ventas_auditoria_segunda_model->Ventas_auditoria_segunda_model_crea_tabla($cmbAnio,$cmbMes);
        foreach ($ventas["auditoria_tabla"] as $venta) { 
            ...
            if ($venta->VentaAuditoriaTipoId==1){
                $tickets_repetidos = $this->ventas_auditoria_segunda_model->ventas_auditoria_segunda_model_tickets_repetidos($venta->VentaId,$cmbAnio,$cmbMes,$venta->DistribuidorId,$venta->VentaUsuarioIdMP,$venta->VentaMontoTicket);
            } else {
                $tickets_repetidos = "&nbsp;";
            }
        ...
        $data['tabla']                    = $lista;
        $tablaauditoriafx = $this->load->view('ventas/ventas_auditoria/ventas_auditoria_segunda/ventas_auditoria_segunda_tabla_view', $data, true);
    ...
    public function ventas_auditoria_segunda_controller_aprobada() {
        ...
        $this->ventas_auditoria_segunda_model->ventas_auditoria_segunda_model_aprobar($VentaAuditoriaId);
        echo json_encode($dato);
    }
    public function ventas_auditoria_segunda_controller_rechazada() {
        $dato['VentaAuditoriaId']   = $VentaAuditoriaId       = $this->input->post('VentaAuditoriaId',true);
        $Observacionid              = $this->input->post('Observacionid',true);
        $dato['fechacambio']        = date('Y-m-d');
        $dato['status']             = 'RECHAZADA';
        $dato['Observacion']        = $this->ventas_auditoria_segunda_model->ventas_auditoria_segunda_model_observacion_descripcion($Observacionid);
        $this->ventas_auditoria_segunda_model->ventas_auditoria_segunda_model_rechazada($VentaAuditoriaId,$Observacionid);
        echo json_encode($dato);
    }
    public function ventas_auditoria_segunda_controller_combo_observaciones() {
        $observaciones = $this->ventas_auditoria_segunda_model->ventas_auditoria_segunda_model_observaciones();
