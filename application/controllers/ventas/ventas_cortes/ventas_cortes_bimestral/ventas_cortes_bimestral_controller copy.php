...
class Ventas_cortes_bimestral_controller extends Base_Controller {
    ...
    public function ventas_cortes_bimestral_controller_creacion() { 
        ...
        $cmb_periodo = $this->input->post('mes',true);
        ...
        if ($this->ventas_cortes_bimestral_model->ventas_corte_bimestral_model_valida_corte_cambio_estatus($cmb_anio,$cmb_periodo,$mes_anterior)==1){ echo 2; return false; }
        if ($this->base_controller_valida_corte(1, $cmb_anio, $cmb_periodo,0)==0){ echo 4; return false; }
        if ($this->ventas_cortes_bimestral_model->ventas_corte_bimestral_model_valida_ventas_auditorias($cmb_anio,$cmb_periodo,$mes_anterior)==1){ echo 3; return false; }
      //  if ($this->ventas_cortes_bimestral_controller_valida_ventas($cmb_anio,$cmb_periodo,$mes_anterior)==0){ echo 2; return false; }
       ...
       // $this->ventas_cortes_bimestral_controller_creacion_perfiles($cmb_anio,$cmb_periodo,$mes_anterior,$corte_id);
        ...
        $this->ventas_cortes_bimestral_controller_visualizacion($cmb_anio,$cmb_periodo,$mes_anterior,$corte_id);
    ...
    private function ventas_cortes_bimestral_controller_creacion_productos_registrados($cmb_anio,$cmb_periodo,$mes_anterior,$corte_id) {
        $ventas = $this->ventas_cortes_bimestral_model->ventas_cortes_bimestral_model_productos_registrados($cmb_anio,$cmb_periodo,$mes_anterior,$corte_id);
        foreach ($ventas as $row) {         
                switch ($row->VentaAuditoriaEstatusId){
                    case 1: $statusauditoria = "APROBADA"; break;
                    case 2: $statusauditoria = "RECHAZADA"; break;
                    default: $statusauditoria = "PENDIENTE"; break;
                }
            $VentaDetalleMonto = 0;
            $VentaDetalleMonto = $row->VentaDetalleCantidad * $row->VentaDetalleLitros ;
            $data = $corte_id.",".$row->DistribuidorDetalleId.",".$row->DistribuidorId.",".$row->DistribuidorDetalleCodigo.",'".utf8_encode(strtoupper($row->DistribuidorDetalleRazonSocial))."','".utf8_encode(strtoupper($row->DistribuidorDetalleNombreComercial))."',".$row->UsuarioId.",".$row->UsuarioDetalleId.",'".utf8_encode(strtoupper($row->nombrepax))."',".$row->TarjetaId.",".$row->TarjetaNumero.",".$row->VentaId.",'".utf8_encode(strtoupper($row->VentaNumeroTicket))."',".$row->VentaMontoTicket.",".$row->ProductoClaseId.",'".utf8_encode(strtoupper($row->ProductoClaseDescripcion))."',".$row->ProductoMarcaId.",'".utf8_encode(strtoupper($row->ProductoMarcaDescripcion))."',".utf8_encode(strtoupper($row->VentaDetalleLitros)).",".$row->VentaDetalleCantidad.",".$VentaDetalleMonto.",".$row->VentaDetalleMonto.",".$row->VentaDetalleTotal.",'".$statusauditoria."','".funciones_strategix_convertir_fecha_hora_actual($row->VentaFechaRegistro)."',".$row->DivisionId.",'".utf8_encode(strtoupper($row->DivisionNombre))."',".$row->PaisId.",'".strtoupper($row->PaisNombre)."'";
