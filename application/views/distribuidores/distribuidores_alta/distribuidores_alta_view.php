    ...
    <section id="alta_distribuidires">
        <div class="panel-title">
            <div class="container">
                <div class="row">
                    <div class="col-lg-12">
                        <h2><?= $this->lang->line('distribuidores_alta_controller_lang_titulo') ?></h2>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        /**************************************************************************************************************************************/
        distribuidores_alta_view_js_combo_regiones();
    ...
    function distribuidores_alta_view_js_combo_oficinas_venta() {
        $('#loader_panel').show();
        $.ajax({
            type: 'POST',
    ...
    function distribuidores_alta_view_js_combo_regiones() {
        ...
        $.ajax({
            type: 'POST',
            url: 'distribuidores/distribuidores_alta/distribuidores_alta_controller/distribuidores_alta_controller_combo_lista_regiones',
            dataType: 'json',
            data: {
                id: 0
            },
    ...
    function distribuidores_alta_view_js_guardar() {
        ...
        Swal.fire({
            ...
            confirmButtonText: '<?= $this->lang->line('distribuidores_alta_controller_lang_confirm_boton_aprobado') ?>',
            cancelButtonText: '<?= $this->lang->line('distribuidores_alta_controller_lang_confirm_boton_rechazado') ?>',
                    ...
                    success: function(data) {
                        switch (data.res) {
                            case 1:
                                Swal.fire({
                                    title: '',
                                    html: '<?= $this->lang->line('distribuidores_alta_controller_lang_guardado_js_msg_swal_texto') ?>',
                                    icon: 'success',
                                    showCancelButton: true,
                                    confirmButtonColor: '#fd7e14',
                                    cancelButtonColor: '#6c757d',
                                    confirmButtonText: '<?= $this->lang->line('distribuidores_alta_controller_lang_guardado_js_msg_swal_aceptar') ?>',
                                    cancelButtonText: '<?= $this->lang->line('distribuidores_alta_controller_lang_guardado_js_msg_swal_cancelar') ?>'
                                }).then((validacionaltaparticipante) => {
                                    if (validacionaltaparticipante.isConfirmed) {
                                        $(location).attr("href", "<?= funciones_strategix_version_url_random_base_url("UsuariosParticipantesAltas") ?>&IDDIS=" + data.DistribuidorId);
                                    } else {
                                        $(location).attr("href", "<?= funciones_strategix_version_url_random_base_url("DistribuidoresAlta") ?>");
                                    }
                                });
                                break;
                            case 2:
