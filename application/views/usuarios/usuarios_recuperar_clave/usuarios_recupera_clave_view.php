            ...
            <div class="row">
                ...
                <div class="col-lg-6" style="padding-left: 50px;">
                    ...
                    <div class="row justify-content-center mt-20 form-login">
                        ...
                        <div class="botones d-flex col-lg-8">
                            <div class="col-lg-6 col-12 center pr-20">
                                <button type="button" id="btn_envio_mail" class="btn btn-black btn-buscar-ancho" pb-role="submit"><span class="iconify" data-icon="bx:bx-mail-send"></span> <?= $this->lang->line('usuarios_recupera_clave_controller_lang_label_button_send') ?></button>
                            </div>
                            <div class="col-lg-6 col-12 center pl-20">
                                <a href="<?php echo funciones_strategix_version_url_random_base_url("login") ?>"><button type="button" class="btn btn-gray btn-buscar-ancho"><?= $this->lang->line('usuarios_recupera_clave_controller_lang_btn_login') ?></button></a>
                            </div>
                            <div class="row">
...
<script>
    $(document).ready(function() {
        $("#frm_recupera_clave").keypress(function(e) {
            if (e.which == 13) {
                return false;
            }
        });
        $('#btn_envio_mail').click(function() {
            usuarios_recupera_clave_controller_lang_envio();
        });

        /********************************************MSG ERROR******************************************************************************************/
        $('#frm_recupera_clave input').on('keyup', function() {
            js_general_limpiar_errores(this);
        });
        $('#frm_recupera_clave input').on('click', function() {
            js_general_limpiar_errores(this);
        });
        $('#frm_recupera_clave select').on('click', function() {
            js_general_limpiar_errores(this);
        });
        $('#frm_recupera_clave input').on('change', function() {
            js_general_limpiar_errores(this);
        });
        /**************************************************************************************************************************************/
    ...
    function usuarios_recupera_clave_controller_lang_envio() {
        grecaptcha.ready(function() {
            grecaptcha.execute('<?= $this->config->item('google_key') ?>', {
                action: '<?= $frm_action ?>'
            }).then(function(token) {
                ...
                if (usuarios_crea_clave_view_chk_email === undefined) {
                    usuarios_crea_clave_view_chk_email = 0;
                }
                if (usuarios_crea_clave_view_chk_whatsapp === undefined) {
                    usuarios_crea_clave_view_chk_whatsapp = 0;
                }
                ...
                $.ajax({
                    ...
                    data: {
                        usuarios_crea_clave_view_txt_email: usuarios_crea_clave_view_txt_email,
                        action: frm_action,
                        token: token,
                        usuarios_crea_clave_view_txt_whatsapp: usuarios_crea_clave_view_txt_whatsapp,
                        usuarios_crea_clave_view_chk_email: usuarios_crea_clave_view_chk_email,
                        usuarios_crea_clave_view_chk_whatsapp: usuarios_crea_clave_view_chk_whatsapp
                    },
                        ...
                        switch (data.res) {
                            case 0:
                                Swal.fire({
                                    ...
                                    text: '<?= $this->lang->line('usuarios_recupera_clave_controller_lang_msg_captcha_erroneo') ?>'
                                });
                                break
                            case 1:
                                Swal.fire({
